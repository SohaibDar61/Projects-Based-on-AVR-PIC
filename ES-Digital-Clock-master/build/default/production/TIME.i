...
void time_update(void){
    ...
    static tByte current_period = (0);



    current_period = current_period + (5);

    current_sec_period = current_sec_period + (5);
            ...
            for(index = sw_plus; index < (3); index++){
                ...
                switch(index){
                    case sw_plus :
                        ...
                        ((switch_getState(sw_plus) == sw_prepressed) && (current_mode != MODE_NORMAL)) ? sw_plus_handler() : 0 ;
                     ...
                     case sw_minus :
                        ...
                        ((switch_getState(sw_minus) == sw_prepressed) && (current_mode != MODE_NORMAL)) ? sw_minus_handler() : 0 ;
                     ...
                     case sw_setting :
                        ...
                        (switch_getState(sw_setting) == sw_prepressed) ? sw_setting_handler() : 0 ;
...
static void sw_minus_handler(void){
    switch(current_mode){
        ...
        case MODE_ADJUST_HOURS:
            current_time.hours--;
            if(current_time.hours == 255){current_time.hours = (24) - 1;}

            break;
        case MODE_ADJUST_MINUTES:
            current_time.minutes--;
            if(current_time.minutes == 255 ){ current_time.minutes = (60) - 1;}
...
static void update_current_time(void){
    ...
    if( ( current_mode == MODE_NORMAL ) && ( current_sec_period >= (1000) )){
        current_sec_period = (0);
        current_time.secounds++;

        if(current_time.secounds == (60)){
            current_time.secounds = (0);
            current_time.minutes++;
