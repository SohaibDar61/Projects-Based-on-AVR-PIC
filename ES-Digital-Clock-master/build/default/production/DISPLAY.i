...
static tWord dot_blink_counter = (0);
static tWord mode_blink_counter = (0);
...
void display_update(void){
    ...
    static tByte current_disp_period = (0);


    current_disp_period += (5);
    dot_blink_counter += (5);
    mode_blink_counter += (5);

    if(current_disp_period < (20)){
        return;
    }

    current_disp_period = (0);
...
static void normal_mode_display(void){
    ...
    ssd_setSymbol(ssd_hour_units , display_time.hours % 10);
    ssd_setSymbol(ssd_hour_tens , display_time.hours / 10);
    if(dot_blink_counter == (1000)){
        dot_blink_counter = (0);
        ssdDot_setState(!ssdDot_getState());
    }
    mode_blink_counter = (0);
}
